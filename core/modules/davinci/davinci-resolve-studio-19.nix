    ...
    stdenv.mkDerivation rec {
      ...
      src = runCommandLocal "${pname}-src.zip"
        rec {
          ...
          outputHash =
            if studioVariant
            #then "sha256-5QIKWORfeynfSYcP8G7u5DY/kbwTp1JmKyvgboSoV5U=" 20 beta
            then "sha256-OTL83suZXt7DxDz+89zIRJD8R25/HZUQMMGlfS+Ow4I="
            else "sha256-2u1gkaL3vdI+4RnPl5bEXE+zeRhg2BzPWjni015ISWI";
        ...
        DOWNLOADID=$(
          curl --silent --compressed "$DOWNLOADSURL" \
            ...
            #| jq --raw-output '.downloads[] | .urls.Linux?[]? | select(.downloadTitle | test("^'"$PRODUCT 20 Beta 1"'( Update)?$")) | .downloadId' 20 beta
        )
